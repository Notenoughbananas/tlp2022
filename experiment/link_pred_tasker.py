...
class Link_Pred_Tasker:
    ...
    def __init__(
        self, args, dataset, temporal_granularity, custom_labeler=None, tlp_labeler=None
    ):
        self.data = dataset
        ...
        self.custom_labeler = custom_labeler
        self.has_time_query = (
            hasattr(args, "has_time_query") and args.has_time_query == True
        )
        self.tlp_labeler = tlp_labeler
        if self.has_time_query:
            assert self.tlp_labeler != None

        if not (
            args.use_2_hot_node_feats
            or args.use_1_hot_node_feats
            or args.use_1_log_hot_node_feats
        ):
    ...
    def build_get_node_feats(self, args, dataset):
        ...
        elif args.use_1_log_hot_node_feats:
            max_deg = max_deg_out
            self.feats_per_node = int(math.log2(max_deg))

            if (
                hasattr(self.args, "min_node_emb_size")
                and self.args.min_node_emb_size > self.feats_per_node
            ):
                min_feat_size = self.args.min_node_emb_size
                self.feats_per_node = min_feat_size

            def get_node_feats(adj):
                return tu.get_1_hot_deg_feats(
                    adj, self.feats_per_node, dataset.num_nodes, apply_log=True
                )
    ...
    def get_sample(self, idx, partition, **kwargs):
        ...
        label_adj = self.get_label_adj(idx, partition, existing_nodes, add_all_edges)
        ...
        if self.has_preprocessor:
            node_features = self.preprocessor.get_node_features(idx)
            link_features = self.preprocessor.get_link_features(idx)
        else:
            link_features = {}
            node_features = {}
    ...
    def get_label_adj(self, idx, partition, existing_nodes, add_all_edges):
        if self.custom_labeler != None:
            return self.custom_labeler.get_label_adj(idx)

        # WSDM cup style prediction
        if self.has_time_query:
            return self.tlp_labeler.get_label_adj(idx)

        label_adj_index = idx + 1
        # label_adj is an edge list of the edges we test on.
        label_adj = tu.get_sp_adj(
            edges=self.data.edges,
            snapshot=label_adj_index,
            weighted=False,
            time_window=1,
