...
)  # --> added for edge_cls (only called in edge_cls tasker, thus useless?)
...
def get_1_hot_deg_feats(adj, max_deg, num_nodes, apply_log=False):
    new_vals = torch.ones(adj["idx"].size(0))
    new_adj = {"idx": adj["idx"], "vals": new_vals}
    degs_out, _ = get_degree_vects(new_adj, num_nodes)

    if apply_log:
        degs_out = (degs_out.type(dtype=torch.float) + 1).log().type(dtype=torch.long)

    degs_out = {
        "idx": torch.cat(
            [torch.arange(num_nodes).view(-1, 1), degs_out.view(-1, 1)], dim=1
        ),
        "vals": torch.ones(num_nodes),
    }
...
def get_sp_adj(
    ...
    d = {"idx": idx, "vals": vals, "time": time}
    if with_edge_features:
        edge_feat = edges["idx"][subset][:, [ECOLS.type]]
        d["type"] = edge_feat
    return d
...
def get_non_existing_edges(
    adj, num_edges, tot_nodes, smart_sampling, existing_nodes=None
):
    ...
    if missing_links > 0:
        ...
        print(
            "WARNING: Negative sampling not equal to requested edges. The negative sampler have failed to supply the right amount of samples."
        )
    return {"idx": edges, "vals": vals}
...
def sample_labels(label_adj, sample_rate):
    ...
    label_adj_link = {
        "idx": label_adj["idx"][link_mask],
        "vals": label_adj["vals"][link_mask],
    }
    label_adj_nolink = {
        "idx": label_adj["idx"][nolink_mask],
        "vals": label_adj["vals"][nolink_mask],
    }
