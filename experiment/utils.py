...
def save_predictions(
    ...
    predictions_filename = "{}{}predictions.csv.gz".format(prefix, str(snapshot_index))

    links = snapshot.t().cpu()
    df = pd.DataFrame(links.numpy(), columns={"source", "target"})
    df["vals"] = snapshot_vals.cpu()
    df["predictions"] = predictions.cpu()

    if dataset.node_id_translator != None:
        # Translate node_ids
        df["source"] = df["source"].map(dataset.node_id_translator["new2old"])
        df["target"] = df["target"].map(dataset.node_id_translator["new2old"])

    print("saving predictions at {}".format(predictions_filename))
    df.to_csv(predictions_filename, compression="gzip", index=False)
...
def prepare_sample(
    sample, args, num_nodes, tasker=None, temporal_granularity=None, only_label_sp=False
):
    ...
    for i, adj in enumerate(sample.hist_adj):
        ...
        sample.hist_vals.append(vals.to(args.device))
        if temporal_granularity == "continuous":
            hist_time = adj["time"].squeeze().t()
            sample.hist_time.append(hist_time.to(args.device))

        if hasattr(args, "pygeom") and args.pygeom == False:
            # Only used for the original implementation of EGCN
            adj_idx = u.sparse_prepare_tensor(adj, torch_size=[num_nodes])
        else:
            adj_idx = adj["idx"].squeeze().t()
        sample.hist_adj[i] = adj_idx.to(args.device)

        if not only_label_sp:
            # Created some problems for reddit_tgn, we don't use this there anyways.
            assert tasker != None
            nodes = tasker.prepare_node_feats(sample.hist_ndFeats[i])

            sample.hist_ndFeats[i] = nodes.to(args.device)
            hist_node_mask = sample.hist_node_mask[i]
            # transposed to have same dimensions as scorer
            sample.hist_node_mask[i] = hist_node_mask.to(args.device).t()

    label_sp = ignore_batch_dim(args, sample.label_sp)

    if args.task in ["link_pred", "edge_cls"]:
        label_sp["idx"] = label_sp["idx"].to(args.device).t()
    else:
...
def get_memory_size(el):
    def tensor_size(t):
        return t.element_size() * t.nelement()

    if type(el) == type(torch.ones(0)):
        return tensor_size(el)
    elif type(el) == type([]):
        return sum([get_memory_size(t) for t in el])
    elif type(el) == type({}):
        return sum([get_memory_size(t) for key, t in el.items()])
    else:
        raise TypeError
