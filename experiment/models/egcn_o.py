...
class EGCN(torch.nn.Module):
    ...
    def forward(self, A_list, Nodes_list, edge_feats_list, nodes_mask_list):
        node_feats = Nodes_list[-1]

        for unit in self.GRCU_layers:
            Nodes_list = unit(A_list, Nodes_list, edge_feats_list, nodes_mask_list)
...
class GRCU(torch.nn.Module):
    def __init__(self, args):
        ...
        self.gcn = GCNConv(self.args.in_feats, self.args.out_feats, cached=False)
        self.reset_param(self.gcn.weight)
    ...
    def forward(self, A_list, node_embs_list, edge_weight_list, mask_list):
        GCN_weights = self.gcn.weight[None, :, :]
        out_seq = []
        for t, edge_index in enumerate(A_list):
            ...
            self.gcn.weight = torch.nn.Parameter(GCN_weights.squeeze())
            node_embs = self.gcn(node_embs, edge_index, edge_weight)
