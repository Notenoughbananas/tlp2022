...
class FeatureReshape(torch.nn.Module):
...
class AttnModel(torch.nn.Module):
    ...
    def forward(self, src, src_t, seq, seq_t, seq_e, mask):
        ...
        # print("src and sec sizes", src.size(), src_t.size(), seq.size(), seq_t.size(), seq_e.size())
        ...
        # # target-attention
        # print("attn q", q.size(), "k", k.size(), 'mask', mask.size())
...
class TGAT(torch.nn.Module):
    def __init__(
        ...
        assert n_feat.shape[1] == e_feat.shape[1]
        ...
        self.update_node_features(n_feat)
        ...
        # Make simple embedding storage
        num_nodes = n_feat.shape[0]
        self.node_embed = torch.FloatTensor(num_nodes, n_feat.shape[1])
        self.node_embed.requires_grad = False
        ...
        self.merge_layer = MergeLayer(
            self.feat_dim, self.feat_dim, self.feat_dim, self.feat_dim
        )
        ...
        if use_time == "time":
            ...
            self.time_encoder = TimeEncode(
                expand_dim=self.n_feat_th.shape[1], device=self.device
            )
        elif use_time == "pos":
            assert seq_len is not None
            self.logger.info("Using positional encoding")
            self.time_encoder = PosEncode(
                expand_dim=self.n_feat_th.shape[1], seq_len=seq_len
            )
        elif use_time == "empty":
        ...
        # self.merge = MergeLayer(self.feat_dim, self.feat_dim, self.feat_dim, args.layer_2_feats) #torch.nn.Bilinear(self.feat_dim, self.feat_dim, 1, bias=True)
        # self.feature_reshape = FeatureReshape(self.feat_dim, self.feat_dim, args.layer_2_feats)
    ...
    Can be used when a new snapshot is added.
    This is necessary because the node features of future snapshots should not be shown to the model.
    Since the node features are one hot encodings not doing this would possibly introduce future leakage.
    """

    def update_node_features(self, n_feat):
    ...
    def forward(self, nodes, times, ndFeats, mask, num_neighbors=20):
        node_embed_large = self.tem_conv(
            nodes.numpy(), times.numpy(), self.num_layers, num_neighbors
        )
        node_embed = self.feature_reshape(
            node_embed_large
        )  # Reshape node embeddings to correct size

        # This ensures that only the last node embedding of each node is kept
        for i, node in enumerate(nodes):
            self.node_embed[node] = node_embed[i]
    ...
    def contrast(
        ...
        # Store the most recent node embeddings
        # print("src", src_embed.size())
        # print("node embed", self.node_embed.size())

        for i, node in enumerate(src_idx_l):
            src_embedding = src_embed[i].detach().clone()
            target_embedding = target_embed[i].detach().clone()
            src_node = node
            target_node = target_idx_l[i]
            self.node_embed[src_node] = src_embedding
            self.node_embed[target_node] = target_embedding
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20):
        ...
        # print("TC layer", curr_layers, "src", src_idx_l.shape, "time", cut_time_l.shape)
        ...
        if curr_layers == 0:
            ...
            src_ngh_node_batch_th = (
                torch.from_numpy(src_ngh_node_batch).long().to(self.device)
            )
            src_ngh_eidx_batch = (
                torch.from_numpy(src_ngh_eidx_batch).long().to(self.device)
            )

            src_ngh_t_batch_delta = cut_time_l[:, np.newaxis] - src_ngh_t_batch
            src_ngh_t_batch_th = (
                torch.from_numpy(src_ngh_t_batch_delta).float().to(self.device)
            )
