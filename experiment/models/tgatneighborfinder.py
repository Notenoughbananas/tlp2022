...
# NeighborFinder from TGAT
class NeighborFinder:
    def __init__(self, dataset, uniform=False):
        """
        adj_list is a list of lists, the first list is a list of nodes, the second is a list of the edges which are connected to that node.
        ...
        src_l = edgeidx[:, 0].tolist()
        dst_l = edgeidx[:, 1].tolist()
        edge_idx_l = range(0, len(edgeidx))  # This serve as the edgeid
        time_l = edgeidx[:, 2].tolist()
        assert len(src_l) == len(dst_l) == len(edge_idx_l) == len(time_l)

        adj_list = self.init_adj_list(src_l, dst_l, edge_idx_l, time_l)
        ...
        print("Init ngh finder", node_idx_l.shape, node_ts_l.shape, edge_idx_l.shape)

        self.off_set_l = off_set_l

        self.uniform = uniform

    def init_adj_list(self, src_l, dst_l, edge_idx_l, time_l):
        ### Initialize the data structure for graph and edge sampling
        # build the graph for fast query
        # Used to initialize neighborfinder
        # Returns a List[List[int]]
        # The outer list is a list of nodes. The inner list is a list of edges which interacted with that node
        max_idx = max(max(src_l), max(dst_l))
        adj_list = [[] for _ in range(max_idx + 1)]

        # edge_idx = torch.tensor(edge_idx_l)
        # edges = torch.cat([src_l.unsqueeze(1), dst_l.unsqueeze(1), edge_idx.unsqueeze(1), time_l.unsqueeze(1)], dim=1)

        for src, dst, eidx, ts in zip(src_l, dst_l, edge_idx_l, time_l):
