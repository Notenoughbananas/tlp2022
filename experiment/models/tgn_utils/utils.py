...
def get_neighbor_finder(dataset, uniform=False):
    edgeidx = dataset.edges["idx"]
    sources = edgeidx[:, 0]
    destinations = edgeidx[:, 1]
    edge_idxs = range(0, len(edgeidx))  # This serve as the edgeid
    timestamps = edgeidx[:, 2]
    assert len(sources) == len(destinations) == len(edge_idxs) == len(timestamps)

    max_node_idx = max(sources.max(), destinations.max())

    adj_list = [[] for _ in range(max_node_idx + 1)]
    for s, d, eid, t in zip(sources, destinations, edge_idxs, timestamps):
        adj_list[s].append((d, eid, t))
        adj_list[d].append((s, eid, t))
