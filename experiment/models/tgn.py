...
from models.tgn_utils.utils import MergeLayer
from models.tgn_utils.memory import Memory
from models.tgn_utils.message_aggregator import get_message_aggregator
from models.tgn_utils.message_function import get_message_function
from models.tgn_utils.memory_updater import get_memory_updater
from models.tgn_utils.embedding_module import get_embedding_module
from models.tgn_utils.time_encoding import TimeEncode
...
class TGN(torch.nn.Module):
    def __init__(
        ...
        self.node_embed = torch.FloatTensor(
    ...
    def compute_temporal_embeddings(
        ...
        if self.use_memory:
            if self.memory_update_at_start:
                ...
                # m = ~torch.isclose(memory[positives], self.memory.get_memory(positives), atol=1e-5)
                # a = memory[positives]
                # b = self.memory.get_memory(positives)
                # print("not close:", a[m], b[m], a[m]-b[m])

                assert torch.allclose(
                    memory[positives], self.memory.get_memory(positives), atol=5e-3
    ...
    def contrast(
        ...
        for i, node in enumerate(source_nodes):
            src_embedding = source_node_embedding[i].detach().clone()
            target_embedding = destination_node_embedding[i].detach().clone()
            src_node = node
            target_node = destination_nodes[i]
            self.node_embed[src_node] = src_embedding
            self.node_embed[target_node] = target_embedding

        return pos_score.sigmoid().squeeze(), neg_score.sigmoid().squeeze()
