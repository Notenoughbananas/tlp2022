...
class splitter:
    ...
    def __init__(
        ...
        self.eval_only_last = eval_only_last
        assert (
        ...
        if (
            ...
            start_offset = args.num_hist_steps - 1
        ...
        if self.eval_only_last is True:
            # Eval only on the very last embeddings.
            # Set splitter to only return the last snapshot.
            # End is now one step past the end of the dataset,
            #  and thus needs to be covered by a custom labeler.
            assert args.custom_labeler == True
            start = end
            end = end + 1
...
class Double_tasker_data_split(Dataset):
    def __init__(self, tasker, start, end, partition, **kwargs):
        ...
        self.end = end
        self.partition = partition
        self.disc_tasker = kwargs["disc_tasker"]
        self.kwargs = kwargs
    ...
    def __getitem__(self, idx):
        ...
        t2 = self.disc_tasker.get_sample(
            idx, partition=self.partition, snapshot_based=True, **self.kwargs
        )
        return t1, t2
