...
class Logger:
    ...
    def log_epoch_done(self):
        ...
        if self.args.use_logfile:
            self.tensorboard_logging(epoch_metrics)
    ...
    def tensorboard_logging(self, epoch_metrics):
        part = self.partition
        e = self.epoch
        self.writer.add_scalar("Loss/{}".format(part), epoch_metrics["loss"], e)
        self.writer.add_scalar("MAP/{}".format(part), epoch_metrics["map"], e)
        self.writer.add_scalar("AUC/{}".format(part), epoch_metrics["auc"], e)
        # self.writer.add_scalar("F1/{}".format(part), epoch_metrics['avg_f1'], e)

    def get_negative_sample_metrics(self, probs, true_classes):
        # If negative sampling is not used,
        # this simulates what a metric would report if negative sampling was used.
